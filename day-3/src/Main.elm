...
During the rush back on Earth, the fuel management system wasn't 
...
on a grid. You trace the path each wire takes as it leaves the central port, 
...
at the central port where they both start, this point does not
...
parseDirection : String -> Maybe String
parseDirection str =
    if List.member str [ "R", "D", "U", "L" ] then
        Just str
...
moveParse : String -> Maybe Move
moveParse str =
    Maybe.map2 Move
        (parseDirection <| String.left 1 str)
        (str
            |> String.dropLeft 1
            |> String.toInt
        )
...
buildCoordinates : Move -> ( Dest, Dict Coord Int ) -> ( Dest, Dict Coord Int )
buildCoordinates { direction, amount } ( ( ( x, y ), pathLength ), dict ) =
    let
        newDest : List Dest
        newDest =
            case direction of
                "U" ->
                    List.range 1 amount
                        |> List.map (\delta -> ( ( x, y + delta ), pathLength + delta ))
                ...
                "R" ->
                    List.range 1 amount
                        |> List.map (\delta -> ( ( x + delta, y ), pathLength + delta ))
        ...
        helper : Int -> Maybe Int -> Maybe Int
        helper newLength existing =
            case existing of
                Just length ->
                    Just length
                ...
                Nothing ->
                    Just newLength
    in
    ( newDest |> List.reverse |> List.head |> Maybe.withDefault ( ( x, y ), pathLength )
    , newDest |> List.foldl (\( coord, length ) dict_ -> Dict.update coord (helper length) dict_) dict
      --, newCoords |> Set.fromList |> Set.union
    )
...
getVisited str =
    str
        |> String.split ","
        |> List.filterMap moveParse
        |> List.foldl buildCoordinates ( ( ( 0, 0 ), 0 ), Dict.empty )
        |> Tuple.second
...
main =
    ( input, input2 )
        |> Tuple.mapBoth getVisited getVisited
        |> (\( a, b ) ->
                Set.intersect (a |> Dict.keys |> Set.fromList) (b |> Dict.keys |> Set.fromList)
                    |> Set.map (\coord -> (Dict.get coord a |> Maybe.withDefault -9999) + (Dict.get coord b |> Maybe.withDefault -9999))
           )
        |> Set.toList
        |> List.minimum
        |> Debug.toString
